VI_GPU_ST_INDEX(e32,true,(
    ...
    // std::cout << "insn:"<<insn.bits()<<";index="<<index[i]<<";imm="<<insn.v_s_simm11() \
    // <<";baseaddr="<<baseAddr<<";basebias="<<baseBias<<";vreg_inx="<<vreg_inx<<";store_addr="<<(baseBias+((baseTid + vreg_inx)<<2))<<std::endl;

    MMU.store_uint32(baseBias+((baseTid + vreg_inx)<<2),P.VU.elt<uint32_t>(2,vs2, vreg_inx));
    
    // MMU.load_int32(baseBias + (vreg_inx << 2), P.VU.elt<uint32_t>(2,vs2, vreg_inx));}
