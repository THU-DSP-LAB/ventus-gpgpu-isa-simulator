...
void write_bytes(std::ofstream &of,char* p,uint64_t size){
  for (size_t i = 0; i < (size+sizeof(uint32_t)-1) / sizeof(uint32_t); ++i) 
    of<<std::setfill('0') << std::setw(8)<<std::hex<<*((uint32_t*)p+i)<<std::endl;
}
int write_files(char *filename,meta_data* meta,std::vector<char*>buffer,std::vector<std::pair<uint64_t,uint64_t>> buffer_info){
  char* chisel_meta=new char[128];
  char* chisel_data=new char[128];
  sprintf(chisel_meta,"%s.meta",filename);
  sprintf(chisel_data,"%s.data",filename);
  std::ofstream outfile_meta(chisel_meta);
    if (!outfile_meta.is_open()) {
        std::cout << "Error opening file" << std::endl;
        return -1;
    }
  std::ofstream outfile_data(chisel_data);
    if (!outfile_data.is_open()) {
        std::cout << "Error opening file" << std::endl;
        return -1;
    }  
  write_bytes(outfile_meta,(char*)meta,sizeof(meta_data));
  uint64_t buffer_num=buffer.size();
  write_bytes(outfile_meta,(char*)(&buffer_num),sizeof(uint64_t));
  for (int i = 0; i < buffer.size(); i++) {  
      write_bytes(outfile_data,buffer[i], buffer_info[i].second);
      write_bytes(outfile_meta,(char*)(&buffer_info[i].first),sizeof(uint64_t));
    }
  for (int i = 0; i < buffer.size(); i++) {  
      write_bytes(outfile_meta,(char*)(&buffer_info[i].second),sizeof(uint64_t));
    }  
  outfile_data.close();
  outfile_meta.close();

  delete[] chisel_data,chisel_meta;
  return 0;
}
...
int main(){
    ...
    data_2[3]=num_workgroups[0];data_2[4]=num_workgroups[1];data_2[5]=num_workgroups[2];
    data_2[9]=0;data_2[10]=0;data_2[11]=0;data_2[12]=0xa0000000;data_2[13]=0x10000000;

    uint32_t data_3[2]={(uint32_t)vaddr_1,(uint32_t)vaddr_2};//buffer base
    
    std::vector<char*> buffer;
    std::vector<std::pair<uint64_t,uint64_t>> buffer_info;
    buffer.push_back((char*)data_0);
    buffer.push_back((char*)data_1);
    buffer.push_back((char*)data_2);
    buffer.push_back((char*)data_3);
    buffer_info.push_back(std::pair(vaddr_1,16*4));
    buffer_info.push_back(std::pair(vaddr_2,16*4));
    buffer_info.push_back(std::pair(vaddr_3,14*4));
    buffer_info.push_back(std::pair(vaddr_4,2*4));
    write_files(filename,&meta,buffer,buffer_info);
